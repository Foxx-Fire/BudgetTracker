...
protocol DailyViewModelProtocol {
...
final class DailyViewModel: DailyViewModelProtocol {
    ...
    var formattedTotal: String {
        ...
        let currencyCode = transactions.first?.currency.code ?? "USD"
        let currency = currencyService.currency(for: currencyCode) ?? .usd
        return CurrencyFormatter.format(amount: total, currency: currency)
    }
    
    func getFormattedAmount(for transaction: Transaction) -> String {
        let currency = currencyService.currency(for: transaction.currency.code) ?? .usd
    ...
    func getCategoryIconName(for transaction: Transaction) -> String {
        ...
        let foundCategory = categoryService.category(for: categoryName.name)
    ...
    func getBankIconName(for transaction: Transaction) -> String {
        ...
        return bankService.bankIcon(for: transaction.bank.bankName)
    }
}
